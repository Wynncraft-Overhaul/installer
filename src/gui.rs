...
let profile = use_resource(move || {
    ...
    println!("DEBUG: Attempting to initialize profile");
    println!("DEBUG: Source: {}", source);
    println!("DEBUG: Branch: {}", branch);
    println!("DEBUG: Launcher path: {:?}", launcher);

    async move { 
        println!("DEBUG: Calling init() function");
        let result = super::init(source, branch, launcher).await;
        
        match &result {
            Ok(profile) => {
                println!("DEBUG: Initialization successful");
                println!("DEBUG: Manifest name: {}", profile.manifest.name);
                println!("DEBUG: Features count: {}", profile.manifest.features.len());
            },
            Err(e) => {
                println!("DEBUG: Initialization FAILED");
